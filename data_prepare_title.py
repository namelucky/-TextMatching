...
class SNLIDatasetTitle(Dataset):
    '''
    注：词汇表的创建只能通过训练集来创建
    '''
    ...
    def __init__(self, mode, length=24,mask=False):
        ...
        # 标签-数值映射
        # 获取分词后的会话文本和标签
        self.s1_list, self.s2_list, self.label_list = self.load_raw_data(mode)
        # S1最大长度
        self.S1_MAX_LEN = 32
        # S2最大长度
        self.S2_MAX_LEN = 32
    ...
    def load_raw_data(self, mode):
        ...
        return s1_list, s2_list, label_list
    ...
    def build_vocab(self):
        ...
        # else:
        #     s1_list, s2_list, _= self.load_raw_data(mode='train')
        #
        # # -----------------
        # # 创建文本词汇表
        # # -----------------
    ...
    def __getitem__(self, idx):
        ...
        :return:
        '''
        s1, s2, label = self.s1_list[idx], self.s2_list[idx], self.label_list[idx]
        ...
        title_token2=torch.cat([s2_token, s1_token], 0)
        ...
        if self.mask:
            return s1_token, s2_token, s1_mask, s2_mask, label
        else:
            return s1_token,s2_token,title_token,title_token2
...
if __name__ == '__main__':
    ...
    train_data = SNLIDatasetTitle('train')
