...
class RelativeMultiHeadAttn(nn.Module):
    def __init__(self, d_model, n_head, dropout, r_w_bias=None, r_r_bias=None, scale=False):
        ...
        # lattice matrix前的训练因子
        self.alpha = torch.nn.Parameter(data=torch.tensor((1.0,)), requires_grad=True)
    ...
    def forward(self, x, mask, lattice_matrix):
        """
        ...
        # ------------------------------
        # max_len = attn.size(2)
        # vaild_mask = torch.ones_like(attn, requires_grad=False)
        # data1 = torch.triu(vaild_mask, diagonal=-2)  # 负数是从主对角线下开始算起
        # data2 = torch.triu(vaild_mask, diagonal=2 + 1)
        # fianl_mask = data1 - data2
        # attn = attn.masked_fill(mask=fianl_mask == 0, value=float(-1e9))
        # print("attn", attn.size())
        # print('lattice_matrix', lattice_matrix[:, None, :, :].size())

        # ----------------------
        # attn_max*0.2
        # ----------------------
        attn_max = attn.max(dim=-1).values
        attn_max = attn_max[:, :, :, None]
        if lattice_matrix!=None:
        # 添加lattice信息
            lattice_matrix = lattice_matrix[:, None, :, :]
            # 取最大值的一半
            attn = attn + 0.2 * attn_max * lattice_matrix

        # -----------------------
        # lattice*alpha
        # -----------------------
        # lattice_matrix = 1 * lattice_matrix[:, None, :, :]
        # attn = attn + lattice_matrix

        # lattice_matrix = self.alpha * lattice_matrix[:, None, :, :]
        # attn = attn + lattice_matrix

        attn = F.softmax(attn, dim=-1)
        # 保存注意力分数分布
        # file_num = len(os.listdir(r'D:\pychram_project\ner\TENer\attn_data\resume\rel\unscale'))
        # if file_num < 10:
        #     # 保存结果
        #     np.save(r'D:\pychram_project\ner\TENer\attn_data\resume\rel\unscale\attn_{}.npy'.format(file_num + 1),
        #             attn.detach().numpy())
...
if __name__ == '__main__':
    batch = 12
    input_len = 18
    mask = torch.ones(12, 18)
    pos_embed = RelativeSinusoidalPositionalEmbedding(128, 0, 1200)
    print(pos_embed(mask).size())
