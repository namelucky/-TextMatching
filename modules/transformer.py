...
class MultiHeadAttn(nn.Module):
    def __init__(self, d_model, n_head, dropout=0.1, scale=True):
        """
    ...
    def forward(self, x, mask, lattice_matrix):
        ...
        # 会自动进行board casting来进行补全
        # mask:(batch, max_len) -> (batch,1,1,max_len)
        attn.masked_fill_(mask=mask[:, None, None].eq(0), value=1e-9)

        # # # 在添加只关联指定相邻位置的元素
        # max_len = attn.size(2)
        # vaild_mask = torch.ones_like(attn, requires_grad=False)
        # data1 = torch.triu(vaild_mask, diagonal=-5)  # 负数是从主对角线下开始算起
        # data2 = torch.triu(vaild_mask, diagonal=5 + 1)
        # fianl_mask = data1 - data2
        # attn = attn.masked_fill(mask=fianl_mask == 0, value=float(-1e9))

        # print(torch.max(attn), self.scale)
        attn = F.softmax(attn, dim=-1)  # batch_size x n_head x max_len x max_len
        # print(torch.max(attn), self.scale)

        # 保存注意力分数分布
        file_num = len(os.listdir('/home/alanlee/recommendation/T_copy/photo'))
        if file_num < 10:
            # 保存结果
            np.save('/home/alanlee/recommendation/T_copy/photo/attn_{}.npy'.format(file_num + 1),
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, mask, lattice_matrix):
        ...
        x = self.self_attn(x, mask, lattice_matrix)
        x = x + residual
        if self.after_norm:
            x = self.norm1(x)
        residual = x
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, mask, lattice_matrix=None):
        """
